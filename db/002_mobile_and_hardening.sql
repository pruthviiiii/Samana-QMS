ALTER TABLE qms.users DROP CONSTRAINT users_role_check;
ALTER TABLE qms.users ADD CONSTRAINT users_role_check CHECK(role IN ('admin','hod','manager','agent','reception','customer','display'));
ALTER TABLE qms.tickets ADD COLUMN public_token uuid NOT NULL DEFAULT gen_random_uuid() UNIQUE;
ALTER TABLE qms.outbox ADD COLUMN revision integer NOT NULL DEFAULT 1;
CREATE OR REPLACE VIEW qms.ticket_view AS SELECT t.id,t.number,t.day,t.sequence,t.request_id,t.lookup_id,t.service_id,t.status,t.customer_id,t.customer_name,t.mobile,t.emirates_id,t.passport_number,t.identifier_type,t.unit_id,t.unit_name,t.project_name,t.booking_number,t.owner_sf_id,t.manager_sf_id,t.assigned_to,t.assigned_at,t.routing_reason,t.escalated,t.created_by,t.created_at,t.called_at,t.started_at,t.closed_at,t.comments,t.version,s.name service_name,s.department,u.name assigned_name,u.counter,t.public_token FROM qms.tickets t JOIN qms.services s ON s.id=t.service_id LEFT JOIN qms.users u ON u.id=t.assigned_to;
...
BEGIN
 ...
 IF NOT p_initial AND t.assigned_to IS NOT NULL THEN
  ...
  IF assigned_user.enabled AND t.service_id=ANY(assigned_user.services) AND assigned_user.online AND assigned_user.last_seen>now()-interval '90 seconds' AND (t.escalated OR now()<=t.created_at+interval '5 minutes') THEN RETURN; END IF;
 END IF;
...
BEGIN
 ...
 ELSIF p_action='call' THEN
  ...
  IF NOT(t.service_id=ANY(u.services)) AND NOT(privileged AND u.sf_id=t.manager_sf_id AND t.service_id='collection') THEN RAISE EXCEPTION 'FORBIDDEN';END IF;
