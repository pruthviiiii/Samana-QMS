...
-- table qms.events
  ...
  CREATE INDEX events_action ON qms.events USING btree (action, id DESC)
...
-- table qms.lookups
  ...
  CREATE INDEX lookups_identifier ON qms.lookups USING btree (identifier_type, identifier_value, created_at DESC)
...
-- table qms.notifications
  ...
  CREATE INDEX notifications_unread ON qms.notifications USING btree (user_id, id DESC) WHERE (read_at IS NULL)
...
-- table qms.outbox
  ...
  CREATE INDEX outbox_ready ON qms.outbox USING btree (status, available_at)
...
-- table qms.rate_limits
  ...
  CREATE UNIQUE INDEX rate_limits_pkey ON qms.rate_limits USING btree (key)
...
-- table qms.services
  ...
  constraint services_priority_not_null NOT NULL priority
  constraint services_priority_range CHECK (((priority >= 0) AND (priority <= 9)))
...
-- table qms.tickets
  ...
  CREATE INDEX tickets_active ON qms.tickets USING btree (status) WHERE (status = ANY (ARRAY['waiting'::text, 'called'::text, 'serving'::text]))
  ...
  CREATE INDEX tickets_customer_name_trgm ON qms.tickets USING gin (customer_name gin_trgm_ops)
  CREATE UNIQUE INDEX tickets_day_service_id_sequence_key ON qms.tickets USING btree (day, service_id, sequence)
  CREATE INDEX tickets_number_trgm ON qms.tickets USING gin (number gin_trgm_ops)
  ...
  CREATE INDEX tickets_unit_name_trgm ON qms.tickets USING gin (unit_name gin_trgm_ops)
  CREATE INDEX tickets_waiting_unassigned ON qms.tickets USING btree (service_id, created_at) WHERE (status = 'waiting'::text)
...
-- table qms.users
  ...
  created_at timestamptz not null default now()
  email text
  rotation int8 not null default nextval('qms.rotation_position'::regclass)
  ...
  constraint users_rotation_not_null NOT NULL rotation
  ...
  CREATE INDEX users_rotation ON qms.users USING btree (rotation) WHERE (enabled AND online)
...
-- sequence qms.rotation_position
...
-- function qms.note_worker_run(p_checked integer)
...
-- function qms.offer_next(p_user uuid)
...
-- function qms.route_maintenance()
...
-- function qms.route_waiting(p_limit integer, p_offset integer)
...
-- function qms.set_service_priority(p_service text, p_priority integer, p_actor uuid)
