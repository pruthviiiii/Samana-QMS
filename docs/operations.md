...
App data is isolated in the `qms` schema of `samana_qms` on the connected Neon project. Tests use `samana_qms_test`. The web app and the worker connect as the restricted role `qms_app`, which can read and write the `qms` tables and run its functions and nothing else; migrations run as the schema owner through `MIGRATE_DATABASE_URL`. `npm run db:app-role` (`scripts/create-app-role.mjs`) creates the role in the database the owner string points at, or refreshes its grants, and prints the `DATABASE_URL` to use. Run it once per database. Roles are shared by every database on the Neon branch, so the second run only adds the grants for that database. Do not publish connection strings or add `.env`, `.dev.vars`, `.analysis`, `.sf`, or generated artifacts to source control.
...
Use the Neon project's backup/PITR controls and choose a recovery window according to the organization's retention policy. To rehearse a restore, create a Neon branch from the production branch at a past timestamp, run `node scripts/migrate.mjs` against it with a connection string for that branch, check the queue state and audit history, then delete the branch. Do not restore over an active production database as a first recovery step.
