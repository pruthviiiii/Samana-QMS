...
// owner through MIGRATE_DATABASE_URL.
//
...
const [owner] = await sql.query('SELECT current_user AS name');
const [existing] = await sql.query('SELECT 1 FROM pg_roles WHERE rolname=$1', [
  role,
]);
if (!existing) {
  ...
  await sql.query(`ALTER ROLE "${role}" PASSWORD '${password}'`);
